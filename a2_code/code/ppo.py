...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        new_logprobs = action_distribution.log_prob(actions)
        # probability ratio comes from importance sampling.
        r_theta = torch.exp(new_logprobs) / torch.exp(old_logprobs)
        loss = -torch.min(r_theta * advantages, torch.clip(r_theta, 1 - self.eps_clip, 1 + self.eps_clip) * advantages).mean()
