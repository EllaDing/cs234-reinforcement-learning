...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        sampled_actions_tensor = distribution.sample()
        log_probs = distribution.log_prob(sampled_actions_tensor).detach().numpy()
        sampled_actions = sampled_actions_tensor.detach().numpy()
        #########          END YOUR CODE.          ############
        if return_log_prob:
...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.categorical.Categorical(logits=self.network(observations))
        #########          END YOUR CODE.          ############
        return distribution
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.MultivariateNormal(self.network(observations), torch.diag(self.std()**2))
        #########          END YOUR CODE.          ############
        return distribution
